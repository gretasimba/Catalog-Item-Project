...
# Home page shows all available categories and up to 6 most recent items
@app.route('/')
...
def showHome():
    ...
    if 'username' not in login_session:
        return render_template(
            'publiccategories.html', categories=categories, items=items)
    else:
        return render_template(
...
def showCategories():
    ...
    if 'username' not in login_session:
        return render_template(
            'publiccategories.html', categories=categories,
            items=items)
    else:
        return render_template(
...
# Add new category, user has to be logged
@login_required
...
# Delete category for logged user
...
# Add new item if user logged
@app.route(
    '/adoptCenter/<int:category_id>/allItems/new/',
    methods=['GET', 'POST'])
@login_required
def newItem(category_id):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'newItem.html', category=category, categories=categories)


# Edit an item if logged
@app.route(
    '/adoptCenter/<int:category_id>/allItems/<int:item_id>/edit',
    methods=['GET', 'POST'])
@login_required
def editItem(category_id, item_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized"
        " to edit items to this category. Please create your"
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'editItem.html', category=category, item_id=item_id,
            item=editedItem, categories=categories)


# Delete an item if logged
@app.route(
    '/adoptCenter/<int:category_id>/allItems/<int:item_id>/delete',
    methods=['GET', 'POST'])
@login_required
def deleteItem(category_id, item_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized"
        " to delete items to this category. Please create your own categoryz"
